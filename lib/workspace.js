...
// ─── Constants ────────────────────────────────────────────────────────────────
const HISTORY_TOKEN_LIMIT = 15000;
const GLOBAL_TOKEN_LIMIT  = 30000;
...
function compactHistory(userId) {
    ...
    if (!ws || !ws.history || ws.history.length <= 1) return;
    ...
    // While over limit, remove from index 1 (keep the first message as anchor)
    // We only remove if there are more than 2 messages.
    while (total > HISTORY_TOKEN_LIMIT && ws.history.length > 2) {
        const removed = ws.history.splice(1, 1)[0]; // removes message at index 1
        if (removed) {
            total -= (removed.tokens || 0);
            console.log(`[Workspace] Compacted history for ${userId}: removed message index 1 (${removed.tokens} tokens). Remaining: ${total}`);
        }
    }

    // Final fallback: if STILL over limit, remove the first message too
    while (total > HISTORY_TOKEN_LIMIT && ws.history.length > 1) {
        const removed = ws.history.shift();
        total -= (removed.tokens || 0);
    }
}
